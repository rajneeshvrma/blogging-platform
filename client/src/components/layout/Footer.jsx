        ...
        <div className="max-w-7xl mx-auto py-12 px-8 bg-glass rounded-xl border border-glass">
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
                <div className="md:col-span-1">
                    <a href="#" onClick={() => setPage({ name: 'home' })} className="text-text-primary font-bold text-2xl tracking-wider">
                        GlassBlog
                    </a>
                ...
                <div className="md:col-span-3 grid grid-cols-2 md:grid-cols-3 gap-8">
                    <div>
                        ...
                        <ul className="mt-4 space-y-2">
                            <li><a href="#" onClick={() => setPage({ name: 'about' })} className="text-base text-text-secondary hover:text-text-primary">About</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'contact' })} className="text-base text-text-secondary hover:text-text-primary">Contact</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'pricing' })} className="text-base text-text-secondary hover:text-text-primary">Pricing</a></li>
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-sm font-semibold text-text-primary tracking-wider uppercase">Resources</h3>
                        <ul className="mt-4 space-y-2">
                            <li><a href="#" onClick={() => setPage({ name: 'explore-blogs' })} className="text-base text-text-secondary hover:text-text-primary">Blog</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'help-center' })} className="text-base text-text-secondary hover:text-text-primary">Help Center</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'community' })} className="text-base text-text-secondary hover:text-text-primary">Community</a></li>
                        </ul>
                    </div>
                    <div>
                        <h3 className="text-sm font-semibold text-text-primary tracking-wider uppercase">Legal</h3>
                        <ul className="mt-4 space-y-2">
                            <li><a href="#" onClick={() => setPage({ name: 'policy' })} className="text-base text-text-secondary hover:text-text-primary">Privacy Policy</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'terms' })} className="text-base text-text-secondary hover:text-text-primary">Terms & Conditions</a></li>
                            <li><a href="#" onClick={() => setPage({ name: 'disclaimer' })} className="text-base text-text-secondary hover:text-text-primary">Disclaimer</a></li>
                        </ul>
                    </div>
                </div>
            </div>
                ...
                <div className="text-sm text-text-secondary mt-4 sm:mt-0">Designed by <span className="relative group text-text-primary font-semibold cursor-pointer"><a href='https://www.linkedin.com/in/rajneeshvrma/'>RAJNEESH </a>
                    <div className="absolute bottom-full mb-2 w-64 bg-glass border border-glass backdrop-blur-md rounded-lg p-4 text-left opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none group-hover:pointer-events-auto -translate-x-1/2 left-1/2 z-10">
                        <div className="flex items-center">
                            <img
                                src="https://media.licdn.com/dms/image/v2/D4D03AQHLRdlJ9q13RQ/profile-displayphoto-shrink_200_200/B4DZgU8dmiH8AY-/0/1752698052051?e=1760572800&v=beta&t=F0iC2E79FKmop-8ZQwcal9g_Ul1fl-qP6bz_Kb6evhA"
                                alt="Raj"
                                class="rounded-full w-[50px] h-[50px]"
                                onerror="this.onerror=null; this.src='https://placehold.co/50x50/778DA9/E0E1DD?text=R';"
                            />
                            <div className="ml-4">
