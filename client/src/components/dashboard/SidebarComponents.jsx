    ...
    return (
        <div className="bg-glass backdrop-blur-xl border border-glass p-6 rounded-2xl shadow-lg text-text-primary animate-fade-in">
            ...
            <form onSubmit={handleSubmit} className="relative">
                <input type="text" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search users or blogs..." className="w-full bg-background/50 dark:bg-white/10 text-text-primary placeholder-text-secondary rounded-full py-2 pl-4 pr-10 focus:border-indigo-500 focus:bg-background/70 outline-none transition" />
...
export const SuggestionsCard = ({ blogs, currentUser, navigateTo }) => {
    const suggestions = blogs.filter(blog => blog.author.id !== currentUser.id).sort(() => 0.5 - Math.random()).slice(0, 4);
        ...
        <div className="bg-glass backdrop-blur-xl border border-glass p-6 rounded-2xl shadow-lg text-text-primary animate-fade-in">
            ...
            <ul className="space-y-4">
                {suggestions.map(blog => (
                    <li key={blog.id}>
                        <button onClick={() => navigateTo('singlePost', blog)} className="w-full text-left group">
